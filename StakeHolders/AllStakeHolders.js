...
const AllStakeHolders = ({ navigation }) => {
...
const getItems = (token) => {
  if (endReached) {
   ...
   const url = EndPoint + `/GetAllStakeHoldersView/?page=${current_page}&page_size=500`
    // console.log(url);
    fetch(url, {
      method: 'GET',
      headers: {
...
{item.MobileNumber && (
       ...
       {/*mwanzo wa button*/}
          <TouchableOpacity
           onPress={() => navigation.navigate("StakeHolder Detail", { ...item, postId: item.id } )}
