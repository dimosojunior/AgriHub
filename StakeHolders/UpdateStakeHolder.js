...
export default function UpdateStakeHolder({ navigation, route }) {
...
useEffect(() => {
  const fetchPostDetails = async () => {
    ...
    if (token) {
      ...
      try {
        const response = await axios.get(`${EndPoint}/RetrieveStakeHoldersView/${postId}/`, {
          headers: {
...
useEffect(() => {
 ...
 if (!emailRegex.test(Email)) {
   ...
   // const date_of_birth = `${year}-${month.padStart(2, '0')}-${day.padStart(2, '0')}`;
    ...
    //setPending(true);
      ...
      if (MobileNumber) {
       ...
       if (Organization) {
        ...
        showAlertFunction('Please enter Organization Name ');
        return;
    }
    ...
    if (Username) {
       ...
       if (Designation) {
        ...
        showAlertFunction('Please enter Designation ');
        return;
    }
    ...
    if (Department) {
        ...
        showAlertFunction('Please enter Department ');
        return;
    }
   ...
   axios.put(EndPoint + `/UpdateStakeHoldersPostView/${postId}/edit/`, formData, {
        headers: {
       ...
       // navigation.goBack();
       navigation.navigate("All StakeHolders");
