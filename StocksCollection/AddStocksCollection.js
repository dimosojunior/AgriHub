...
export default function AddStocksCollection({ navigation, route }) {
...
const [showRegions, setShowRegions] = useState(false);
 ...
 //  }
  ...
  const DateDispatchToCentre = `${year3}-${month3.padStart(2, '0')}-${day3.padStart(2, '0')}`;
   ...
   axios.post(EndPoint + `/AddNewStocksCollectionView/?full_name=${full_name}&Education=${Education}&date_of_birth=${date_of_birth}&country=${country}&region=${region}&district=${district}&ward=${ward}&gender=${gender}&PARTICIPATION_GROUPS=${PARTICIPATION_GROUPS}&FarmersOwnership=${FarmersOwnership}&Username=${Username}&Email=${Email}&reg_no=${reg_no}&NidaNo=${NidaNo}&MobileNumber=${MobileNumber}&TelNumber=${TelNumber}&FarmerSize=${FarmerSize}`, formData, {
        headers: {
        ...
        setPending(false);
        showAlertFunction("New stock information is added successfull");
...
<LinearGradient colors={['#015d68', '#000']} style={globalStyles.container}>
  ...
  <View style={globalStyles.loaderOverlay}>
    <View style={globalStyles.loaderContent}>
      ...
      <Text style={globalStyles.loaderText}>Adding stock</Text>
...
>Add New Stock informations of {full_name}</Text>
