...
const AllStocksCollection = ({ navigation }) => {
...
const getItems = (token) => {
  if (endReached) {
   ...
   const url = EndPoint + `/GetAllStocksCollectionView/?page=${current_page}&page_size=500`
    // console.log(url);
    fetch(url, {
      method: 'GET',
      headers: {
...
const CartCard = ({item, index}) => {
 ...
 return (
    ...
    style={globalStyles.OverdoseProductsContainer}
     onPress={() => navigation.navigate('View Stocks Collection', { ...item, postId: item.id })}
...
{/*mwanzo kwaajili ya kupress order*/}
 ...
 <Pressable
        ...
        <TouchableOpacity
         //onPress={makeOrder}
              onPress={() =>
        navigation.navigate('Stocks Collection Cart Items')}
...
export default AllStocksCollection;
