...
const StocksCollectionOrders = ({navigation}) => {
...
const getProducts = () => {
  if (endReached) {
    ...
    setIsLoading(true);
    const url = EndPoint + `/StocksCollectionOrder/?page=${current_page}&page_size=2`;
     ...
     {item.total_price > 0 && (
      ...
      style={globalStyles.OverdoseCartItemsContainer} >
       ...
       {item.closed_order_state == true ? (
        ...
        <TouchableOpacity 
        onPress={() => navigation.navigate('Stocks Collection Orders Items', item)}
...
{orders && orders.length > 0 ? (
        ...
        >
          ...
          <TouchableOpacity
         // onPress={makeOrder}
        onPress={() => navigation.navigate('All Stocks Collection')}
...
export default StocksCollectionOrders;
