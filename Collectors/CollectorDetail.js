...
const CollectorDetail = ({navigation, route}) => {
...
const [KiasiChaRejeshoChaSiku, setKiasiChaRejeshoChaSiku] = useState(0);
 ...
 const handleDeletePost = async () => {
    ...
    try {
      await axios.delete(EndPoint + `/DeleteCollectorsPostView/${postId}/delete/`, {
        headers: {
...
<View style={[globalStyles.TaarifaBinafsiMainContainer,
 ...
 {CenterName && CenterName.CenterName && (
       <Text style={globalStyles.TaarifaBinafsiSimuYaMteja}>
     Center Name: {CenterName.CenterName}    
      </Text>
      )}
...
{((userData && userData.is_admin === true) || (userData && userData.username === Username))  && (
        ...
        // }}
          ...
          onPress={() => navigation.navigate("Update Collector", 
            {
               postId,
                id,
                full_name,
