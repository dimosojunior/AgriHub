...
export default function AddCollectors({ navigation }) {
...
const [showRegions, setShowRegions] = useState(false);
 ...
 const handleCheckboxChange = (id) => {
  ...
  //         showAlertFunction('Please enter CenterName level.');
  //         return;
  //       }
 ...
 if (!emailRegex.test(Email)) {
   ...
   // const date_of_birth = `${year}-${month.padStart(2, '0')}-${day.padStart(2, '0')}`;
    ...
    // } else {
    //     showAlertFunction('Please enter Physical location ');
    //     return;
    // }
...
// if (!selectedCenterName) {
//           showAlertFunction('Please select collection point name.');
//           return;
//         }
   ...
   axios.post(EndPoint + `/AddNewCollectorsView/`, formData, {
        headers: {
        ...
        setPending(false);
        showAlertFunction("New Collector is added successfull");
        //console.log("Well");
...
<LinearGradient colors={['#015d68', '#000']} style={globalStyles.container}>
  ...
  <View style={globalStyles.loaderOverlay}>
    <View style={globalStyles.loaderContent}>
      ...
      <Text style={globalStyles.loaderText}>Adding new collector</Text>
