...
const AllCollectors = ({ navigation }) => {
...
const getItems = (token) => {
  if (endReached) {
   ...
   const url = EndPoint + `/GetAllCollectorsView/?page=${current_page}&page_size=500`
    // console.log(url);
    fetch(url, {
      method: 'GET',
      headers: {
...
{item.MobileNumber && (
       ...
       {/*mwanzo wa button*/}
          <TouchableOpacity
           onPress={() => navigation.navigate("Collector Detail", { ...item, postId: item.id } )}
...
return (
  ...
  <View style={globalStyles.loaderOverlay}>
    <View style={globalStyles.loaderContent}>
      ...
      <Text style={globalStyles.loaderText}>Loading Collectors...</Text>
