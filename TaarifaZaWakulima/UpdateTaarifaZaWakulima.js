...
export default function UpdateTaarifaZaWakulima({ navigation, route }) {
...
useEffect(() => {
  const fetchPostDetails = async () => {
    ...
    if (token) {
      ...
      try {
        const response = await axios.get(`${EndPoint}/RetrieveTaarifaZaWakulimaView/${postId}/`, {
          headers: {
       ...
       setProduce(data.Produce.toString());
       setEstimatedProduce(data.EstimatedProduce.toString());
       setEstimatedWeightPerBag(data.EstimatedWeightPerBag.toString());
       setFarmPricePerBag(data.FarmPricePerBag.toString());
       setStorage(data.Storage.toString());
       setQuantityStored(data.QuantityStored.toString());

      // setHarverstedOn(data.HarverstedOn.toString());
       //setDateDispatchToCentre(data.DateDispatchToCentre.toString());
       setAcreage(data.Acreage);
       setCropCategory(data.CropCategory);
      ...
      setDay2(data.HarverstedOn ? new Date(data.HarverstedOn).getDate().toString() : '');
      setMonth2(data.HarverstedOn ? (new Date(data.HarverstedOn).getMonth() + 1).toString() : '');
      setYear2(data.HarverstedOn ? new Date(data.HarverstedOn).getFullYear().toString() : '');
      
       setDay3(data.DateDispatchToCentre ? new Date(data.DateDispatchToCentre).getDate().toString() : '');
      setMonth3(data.DateDispatchToCentre ? (new Date(data.DateDispatchToCentre).getMonth() + 1).toString() : '');
      setYear3(data.DateDispatchToCentre ? new Date(data.DateDispatchToCentre).getFullYear().toString() : '');
...
useEffect(() => {
 ...
 //  }
  ...
  // }
   ...
   axios.put(EndPoint + `/UpdateTaarifaZaWakulimaPostView/${postId}/edit/`, formData, {
        headers: {
        ...
        setPending(false);
        showAlertFunction("informations updated successfull");
...
>Update informations of {full_name}</Text>
