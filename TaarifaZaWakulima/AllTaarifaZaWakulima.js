...
const AllTaarifaZaWakulima = ({ navigation }) => {
...
const getItems = (token) => {
  if (endReached) {
   ...
   const url = EndPoint + `/GetAllTaarifaZaWakulimaView/?page=${current_page}&page_size=500`
    // console.log(url);
    fetch(url, {
      method: 'GET',
      headers: {
...
const CartCard = ({item, index}) => {
 ...
 return (
    ...
    style={globalStyles.OverdoseProductsContainer}
     onPress={() => navigation.navigate('View Taarifa', item)}

       // onPress={() => navigation.navigate("View Taarifa", 
       //      {
       //        // postId,
       //          id,
       //          full_name,
      ...
      />
       ...
       <Image
        source={{
          uri: EndPoint + '/' + item.CropType.CropImage,
        }}
        style={globalStyles.OverdoseDiscountProductsImage}
      />
      ...
      >
        ...
        >
          Harversted On: {formatDate(item.HarverstedOn)}
        </Text>
        )}
 ...
 if (item.CropType.Crops.toLowerCase().includes(input.toLowerCase())) {
...
>Farm Cultivation Informations for all farmer's crops</Text>
...
export default AllTaarifaZaWakulima;
