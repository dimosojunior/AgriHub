...
export default function UpdateTransporter({ navigation, route }) {
...
useEffect(() => {
  const fetchPostDetails = async () => {
    ...
    if (token) {
      ...
      try {
        const response = await axios.get(`${EndPoint}/RetrieveTransportersView/${postId}/`, {
          headers: {
...
useEffect(() => {
 ...
 if (!emailRegex.test(Email)) {
   ...
   // const date_of_birth = `${year}-${month.padStart(2, '0')}-${day.padStart(2, '0')}`;
    ...
    if (Username) {
       ...
       if (RegNo_TinNo) {
        ...
        showAlertFunction('Please enter Reg/Tin number ');
        return;
    }
   ...
   axios.put(EndPoint + `/UpdateTransportersPostView/${postId}/edit/`, formData, {
        headers: {
