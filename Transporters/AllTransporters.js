...
const AllTransporters = ({ navigation }) => {
...
const getItems = (token) => {
  if (endReached) {
   ...
   const url = EndPoint + `/GetAllTransportersView/?page=${current_page}&page_size=500`
    // console.log(url);
    fetch(url, {
      method: 'GET',
      headers: {
...
{item.MobileNumber && (
       ...
       {/*mwanzo wa button*/}
          <TouchableOpacity
           onPress={() => navigation.navigate("Transporter Detail", { ...item, postId: item.id } )}
