...
export default function RegisterTransporter({ navigation }) {
...
useEffect(() => {
  ...
  const [RegNo_TinNo, setRegNo_TinNo] = useState('');
 ...
 if (!emailRegex.test(Email)) {
   ...
   // const date_of_birth = `${year}-${month.padStart(2, '0')}-${day.padStart(2, '0')}`;
    ...
    //setPending(true);
      ...
      if (MobileNumber) {
       ...
       if (ContactPerson) {
        ...
        showAlertFunction('Please enter contact person ');
        return;
    }
    ...
    if (Username) {
       ...
       if (RegNo_TinNo) {
        formData.append('RegNo_TinNo', RegNo_TinNo);
    } else {
        showAlertFunction('Please enter Registration / Tin number ');
        return;
    }
    ...
    if (Location) {
        ...
        showAlertFunction('Please enter office location ');
        return;
    }
   ...
   axios.post(EndPoint + `/AddNewTransportersView/`, formData, {
        headers: {
        ...
        setPending(false);
        showAlertFunction("New transporter is added successfull");
        //console.log("Well");
...
>Add New Transporter</Text>
  ...
  <Text style={styles.label}>Reg/Tin No</Text>
      <TextInput value={RegNo_TinNo} 
      onChangeText={setRegNo_TinNo}
      keyboardType="numeric" 
      style={styles.input} 
      placeholder="reg / tin no" 
      placeholderTextColor="wheat"
      />
  ...
  <View style={styles.dropdownList}>
    {districts.map((item, index) => (
      ...
      <View style={styles.row}>
        <TouchableOpacity
          style={[styles.genderButton, Type === 'company' && styles.selectedGender]}
          onPress={() => setType('company')}
        >
          <Text>Company</Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={[styles.genderButton, Type === 'individual' && styles.selectedGender]}
