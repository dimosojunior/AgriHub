...
export default function UpdateBuyer({ navigation, route }) {
...
useEffect(() => {
  const fetchPostDetails = async () => {
    ...
    if (token) {
      ...
      try {
        const response = await axios.get(`${EndPoint}/RetrieveBuyersView/${postId}/`, {
          headers: {
...
useEffect(() => {
 ...
 if (!emailRegex.test(Email)) {
  ...
  //  const date_of_birth = `${year}-${month.padStart(2, '0')}-${day.padStart(2, '0')}`;
   ...
   axios.put(EndPoint + `/UpdateBuyersPostView/${postId}/edit/`, formData, {
        headers: {
       ...
       // navigation.goBack();
       navigation.navigate("All Buyers");
...
<LinearGradient colors={['#015d68', '#000']} style={globalStyles.container}>
  ...
  <View style={globalStyles.loaderOverlay}>
    <View style={globalStyles.loaderContent}>
      ...
      <Text style={globalStyles.loaderText}>Updating buyer</Text>
